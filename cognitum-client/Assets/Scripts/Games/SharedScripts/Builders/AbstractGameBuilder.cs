...
public abstract class AbstractGameBuilder : MonoBehaviour
{
  ...
  {
    OnSetAnswer?.Invoke(answerData);  // Вызов события через метод
  }
  ...
  {
    // т.к. не зависит от платформы и версии ,как обычный GetHashCode, вернёт базовое зерно для игры
    using MD5 md5 = MD5.Create();
    string word = NameGame + countGame;
    byte[] hashBytes = md5.ComputeHash(Encoding.UTF8.GetBytes(word));

    // берём только 3 байта
    BaseSeed = (hashBytes[0] << 16) | (hashBytes[1] << 8) | hashBytes[2];

    // если это не контрольная игра, то один раз создадим рандом на всю игру на базовом сиде + рандом
